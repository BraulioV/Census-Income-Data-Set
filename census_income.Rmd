...
Como vemos en la gráfica, con $m=2$ obtenemos el modelo óptimo con menor error, por tanto, en vez de quedarnos con el modelo generado anteriormente con $m=3$, nos quedamos con el mejor obtenido según el error _Out-Of-Bag_.
...
Como vemos, el error en la clase `>50K` se ha reducido muchísimo con este modelo. Y el error obtenido en la `<=50K` ha subido pero se encuentra dentro de un límite aceptable. Por tanto, podemos concluir que SVM ha conseguido equilibrar ambas clases, aunque también serían necesarias varias operaciones sobre los datos para conseguir que estén equilibrados y reducir el error.
...
```{r}
set.seed(1)
model_knn <- kknn(formula = income ~ ., train = adult.train.clean.norm, test = adult.test.clean.norm, 
     k = best_model_knn$best.parameters$k, kernel = best_model_knn$best.parameters$kernel)
...
```{r}
cat("Error obtenido en la clase <=50K: ", 
    97200/length(adult.test.clean$income[adult.test.clean$income=="<=50K"]))
cat("Error obtenido en la clase >50K: ", 
    139100/length(adult.test.clean$income[adult.test.clean$income==">50K"]))
```

Por tanto, la conclusión obtenida sobre éste modelo es, que a pesar de haber estado limitado por la implementación del paquete de `R`, hemos obtenido un resultado muy competitivos y de una alta calidad, tanto en cuanto a error fuera de la muestra como error en cada clase y equilibrio en clases desbalanceadas, ya que éste modelo ha conseguido disminuir el error de la clase `>50K` por debajo del 40% sin aumentar de forma considerada el error en la clase `<=50K`.
...
En la Tabla 1 vemos una comparativa de los modelos obtenidos. El modelo con menor $E_{out}$ ha sido _Support Vector Machine_ con un kernel radial. Éste modelo también ha conseguido clasificar ejemplos de la clase minoritaria, `>50K`, con menos del 50% de error, aunque ha sido superado por la _Red Neuronal_, que lo ha conseguido con un error menor al 40%. Ahora bien, el modelo _Red Neuronal_ ha jugado con desventaja, al no poder ajustar uno de los parámetros que más flexibilidad da al modelo: el númerod de capas ocultas. A pesar de esta desventaja, ha logrado ser el segundo mejor modelo.

El peor modelo ha sido la _Regresión logística_, ya que ha obviado la clase minoritaria completamente. Éste modelo ha obtenido un 24% de error, ya que ha clasificado de forma correcta todos los ejemplos de la clase `<=50K` y sólo ha fallado en los de la clase `>50K`. Al representar los datos de esta última clase la minoría, el número obtenido es relativamente bajo pero el modelo obtenido como tal es muy malo. El modelo _Random Forest_ también ha hecho una clasificación bastante mala, ya que el error obtenido en la clase `>50K` ha sido demasiado alto. Por tanto, al no haber podido trabajar correctamente con datos desbalanceados estos dos modelos han obtenidos los peores resultados.

Por último, el modelo del _Vecino más cercano_ con base radial ha obtenido buenos resultados, pero en comparación con _Support Vector Machine_ o _Red Neuronal_ ha obtenido un error mayor tanto fuera de la muestra como en la clase minoritaria. Ha obtenido un error menor en `<=50K` que _Red Neuronal_, 8,34 frente a 8,65, pero esta diferencia no es significativa.
