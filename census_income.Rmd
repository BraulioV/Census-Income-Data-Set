...
Como hemos dicho antes, las variables que tienen valores perdidos son _Workclass_, _Occupation_ y _Country_. Vamos a hacer tres modelos diferentes de red neuronal, usando los mismos parámetros que se han usado anteriormente.
...
norm = NULL
workclass.nnet = nnet(formula = workclass ~ ., maxit=10000,
    data = adult.train.clean.norm, size = 10, decay=0.1,trace=F)
pred_workclass = predict(object = workclass.nnet, newdata = adult.train.norm[is.na(adult.train$workclass),], type="class")
adult.train$workclass[is.na(adult.train$workclass)] = pred_workclass
```

## Modelo para recuperar las variables de *occupation*

Para recuperar los datos de esta variable, crearemos un nuevo dataset que llamaremos *adult.train.occupation.lost*, donde almacenaremos las instancias en el conjunto de train que tienen datos perdidos en la variable *occupation*. 

```{r}
adult.train.occupation.lost = adult.train[is.na(adult.train$occupation) == T,]
adult.test.occupation.lost = adult.test[is.na(adult.test$income) == T,]
norm = normalizar_maxmin(data = adult.train.occupation.lost,  
  data_test = adult.test.occupation.lost)
adult.train.occupation.lost.norm = norm[[1]]
adult.test.occupation.lost.norm = norm[[2]]

outOfSampleErrorOccupation <- function(model, newdata = adult.test.clean.norm,  
    printTable = T, type = "class"){
  set.seed(1)
  ypred = predict(object = model, newdata = newdata, type=type)
  if(printTable)
    print(table(predict=ypred, truth=newdata$occupation))
  cat("Eout = ",mean((ypred != newdata$occupation)*1))
}
```

```{r}
set.seed(1)
model.nnet.occupation = nnet(formula = occupation ~ ., maxit=10000, 
    data = adult.train.clean.norm, size = 8, decay=0.5,trace=T)
outOfSampleErrorOccupation(model.nnet.occupation, adult.test.clean.norm, type="class")
y = predict(model.nnet.occupation, newdata = adult.train.occupation.lost.norm, type = "class", )
```
