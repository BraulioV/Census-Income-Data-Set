...
glmModel = glm(income ~ ., data = adult.train.clean, family = "binomial")
predictorGLM(glmModel, pintar=F)
```

Como podemos ver, el error que genera la regresión lineal, tomando todas las variables como predictoras, genera un error de aproximadamente el 24%. Como vemos, el error generado por la regresión logística es bastante alto y por ello, vamos a tratar de mejorar el modelo, haciendo uso de la validación cruzada.

```{r}
library(boot)

cvGlmModel = cv.glm(data = adult.train.clean,  glmfit = glmModel, K = 5)
```
...
La _Red Neuronal_ es un modelo muy extendido y con mucha flexibilidad en cuanto a la parametrización del modelo. Es cierto que puede llegar a tener algo de sobreajuste, pero podemos usar regualización con _early stopping_ o _weight decay_ para evitar esto. 

El número de _unidades ocultas_ de la _Red Neuronal_ es uno de los parámetros con el que más pruebas haremos, ya que con un número grande de capas tenemos la suficiente flexibilidad como para poder capturar la función $g$ que mejor se ajusta a los datos.

## Aplicación del modelo de _Red Neuronal_

Para aplicar el modelo, vamos a seguir las indicaciones dadas en [@redneuronalr] para complementar la teoría aprendida en la asignatura.
...
```{r}
normalizar_maxmin <- function(data=adult.train.clean, numbercols = c("age","fnlwgt","capital.gain","capital.loss","hours.per.week")) {
    # nos quedamos sólo con las columnas numéricas
    cols_numericas = subset(data, select=numbercols)
    # calculamos el máximo y el mínimo de cada columna
    maxs = apply(X=cols_numericas, MARGIN=2, FUN=max)
    mins = apply(X=cols_numericas, MARGIN=2, FUN=min)
    # aplicamos el escalado
    datos_normalizados_numericos = as.data.frame(scale(x=cols_numericas, center=mins, scale=maxs))
    # modificamos las columnas del dataset original. Como en R los parámetros se 
    # pasan por copia esto no afecta a los datos originales.
    for (c in numbercols)
        data[c] = datos_normalizados_numericos[c]
    data # devolvemos el dataset con las columnas numéricas normalizadas.
}

adult.train.clean.normalized = normalizar_maxmin()
adult.test.clean.normalized = normalizar_maxmin(data=adult.test.clean)
```
