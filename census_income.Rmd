...
predice_na <- function(attr, noselectvars, test_dataset = adult.test.norm, 
    ...
    model <- nnet(x = subset(dataset, select=c(-which(colnames(dataset)==noselectvars[1]), 
                -which(colnames(dataset)==noselectvars[2]), -which(colnames(dataset)==attr))),
                y = dataset[,attr],maxit = 100, size = 8, decay = 1, trace = F)
    pred_train <- predict(object = model, newdata = subset(train_dataset[is.na(train_dataset[,attr]),],
                        select=c(-which(colnames(dataset)==noselectvars[1]), 
                    -which(colnames(dataset)==noselectvars[2]))), type = "class")
