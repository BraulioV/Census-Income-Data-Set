...
* ___Regularización___: al tener tantos pesos, las redes neuronales pueden sobreajustar los datos en un mínimo local. Para evitar esto, como hemos dicho antes, podemos usar _early stopping_ o _weight decay_. En este caso vamos a usar _weight decay_

* ___Número de capas ocultas___: es recomendable tener entre 5 y 100 capas, para que el modelo pueda tener flexibilidad suficiente. La regularización se encarga de dejar a 0 aquellos pesos que lleven a sobreajusar el modelo. El problema es que, debido a que el paquete sólo permite usar una única capa oculta, no vamos a poder configurar este parámetro y obtendremos unos resultados bastante malos. Hay otros paquetes que sí permiten configurar el número de capas ocultas, pero no permiten usar datos categóricos.

* __Número de unidades en la capa oculta__: lo que sí podemos configurar es el número de unidades en la capa oculta.
...
# tune.nnet(income ~ ., data = adult.train.clean.norm,
#           size = 1:5, decay = c(0.01,0.1,0.5))
model.nnet <- nnet(formula = income ~ ., data = adult.train.clean.norm, size = 5, decay=0.01)
