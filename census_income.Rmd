...
predice_na <- function(attr, noselectvars, test_dataset = adult.test.norm, dataset = adult.train.norm) {
    # el vector no selectvars siempre tendrá dos variables string
    model <- nnet(x = subset(dataset, select=c(-which(dataset) == noselectvars[1], 
                -which(dataset) == noselectvars[2])), y = dataset[,attr],
                maxit = 100, size = 8, decay = 1, trace = F)
    pred_train <- predict(object = model, newdata = subset(dataset[is.na(dataset[,attr]),],
                        select=c(-which(dataset) == noselectvars[1], 
                        -which(dataset) == noselectvars[2])), type = "class")
...
workclass = predice_na(attr = "workclass", noselectvars = c("occupation", "country"))
