...
getPerfomance <- function(model, norm = F, newdata = adult.test.clean, nnetOrGlm=F,...){
  set.seed(1)
  if(!nnetOrGlm)
...
plot(getPerfomance(model = svmModelReg_09, type = "prob", probability = T),lwd=2,col=3,add=T)
# plot(getPerfomance(model_knn$fitted.values),lwd=2,col=4,add=T)
plot(getPerfomance(model = nNetModel, newdata = adult.test.clean.norm, type="raw"),lwd=2,col=4,add=T)
