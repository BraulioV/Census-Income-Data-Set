...
rf.clean = randomForest(x=subset(adult.train.clean, select = -income), 
    y = adult.train.clean$income, importance = T, mtry = sqrt(ncol(adult.train.clean)-1))
print(rf.clean)

rf.tuned = tuneRF(x=subset(adult.train.clean, select=-income), y=adult.train.clean$income, doBest=T)

ypred = predict(rf.tuned, subset(adult.test.clean, select = -income))
