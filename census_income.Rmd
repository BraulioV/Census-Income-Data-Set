...
getPerfomance <- function(model, newdata = adult.test.clean, svmPred = F, nnetOrGlm=F, calculatePred = T,...){
    set.seed(1)
    if(calculatePred) {
...
plot(getPerfomance(model_knn$prob[,2], newdata = adult.test.clean.norm, calculatePred = F),lwd=2,col=5,add=T)
plot(getPerfomance(model = model.nnet, newdata = adult.test.clean.norm,
  type="raw", nnetOrGlm = T),lwd=2,col=6,add=T)
abli>ne(a=0,b=1,lwd=2,lty=2,col="gray")
