...
En este caso, como hiperparámetro del modelo tenemos el parámetro *cost*, que controla la regularización del problema. Esta regularización de los datos, se realiza haciendo uso de la formulación de Lagrange.
...
### Optimizando el modelo SVM 

Para intentar mejorar el modelo, haremos una versión del modelo $SVM$ que utilizará la función `tune` para ir probando con distintos valores para cada uno de los parámetros, devolviéndonos el mejor modelo encontrado, junto con información del valor de los parámetros usados. Además, en esta versión incluiremos regularización, para intentar generalizar mejor y reducir el sobreajuste.

tune.svm(formula = income ~., data = adult.train.clean, kernel = "radial", k = 15,)

```{r}
svmModelTuned = tune(method = svm, income ~., data=adult.train.clean, kernel = "radial",
  ranges = list(cost=c(0.001,0.01,0.1,1,5,10,100,1000)),
  costs=(-1:1), gamma = (1/14):10, epsilon=seq(0.01, 1, 0.05))
...
```{r}
cat("Error obtenido en la clase <=50K: ", 94700/length(adult.test.clean$income[adult.test.clean$income=="<=50K"]))
cat("Error obtenido en la clase >50K: ", 154100/length(adult.test.clean$income[adult.test.clean$income==">50K"]))
```
