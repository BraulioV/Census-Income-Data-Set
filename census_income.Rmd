...
adult.train$country = as.character(adult.train$country)
adult.train$education = as.character(adult.train$education)
adult.train$marital.status = as.character(adult.train$marital.status)
adult.train$sex = as.character(adult.train$sex)
adult.train$relationship = as.character(adult.train$relationship)
adult.train$occupation = as.character(adult.train$occupation)

adult.test$workclass = as.character(adult.test$workclass)
adult.test$country = as.character(adult.test$country)
adult.test$education = as.character(adult.test$education)
adult.test$marital.status = as.character(adult.test$marital.status)
adult.test$sex = as.character(adult.test$sex)
adult.test$relationship = as.character(adult.test$relationship)
...
Para comprobar que a la hora de ajustar los modelos no tendríamos ningún problema con los datos de test y training, hemos comparado los distintos factores que tienen ambos conjuntos, para ver si hay alguno distinto:

```{r}
v = sapply(X = 1:ncol(adult.train.clean), FUN = function(x){
    if (is.factor(adult.train.clean[,x]))
        levels(adult.test.clean[,x]) %in% levels(adult.train.clean[,x])
    })
names(v) = names(adult.train.clean)
print(v)
```

Como vemos, los nombres de los factores de la variable `income` no coinciden en los datos de test y en los de entrenamiento:

```{r}
print(levels(adult.train.clean$income))
print(levels(adult.test.clean$income))
```

La única diferencia entre ambos es que los factores en los datos de test acaban con un punto al final. Para solucionar este problema, cambiamos la etiqueta de los factores de los datos de test:

```{r}
levels(adult.test.clean$income) = levels(adult.train.clean$income)
print(levels(adult.test.clean$income))
```
